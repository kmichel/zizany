...
namespace zizany {
    ...
    void
    previews_delta::print_action(json_writer &writer) const {
        writer.add_string(operation == delta_set_operation::add ? "add preview" : "remove preview");
    }

    void
    previews_delta::print_details(json_writer &writer) const {
        writer.start_object();
